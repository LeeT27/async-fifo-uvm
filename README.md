...
## Features
- **Dual-Clock Design:** Parameterized $16 \times 8$ SystemVerilog FIFO using Gray-code pointers and 2-stage synchronizers for safe clock domain crossing
- **Flag Logic:** Full and empty flag generation using synchronized Gray-code pointers to prevent overflow and underflow across asymmetric clocks
- **UVM Environment:** Full UVM architecture (driver, monitor, agent) bound via virtual interfaces to generate 1,000+ constrained-random transactions
- **Auto-Checking & SVAs:** TLM scoreboard for automated payload verification alongside 4 concurrent SVAs simulated in Synopsis VCS to enforce correct reads/writes
- **Full Coverage:** Reached 100% code and functional coverage across clock ratio variations and corner cases in Vivado
